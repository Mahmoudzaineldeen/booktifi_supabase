...
BEGIN
  ...
  -- Use available_capacity from slots table (already accounts for bookings)
  -- Subtract locks to get truly available capacity
  ...
  -- Note: slots table uses available_capacity and original_capacity
  -- We update available_capacity which is used for booking checks
  IF v_old_slot_id_final != p_new_slot_id THEN
    UPDATE slots
    SET 
      available_capacity = LEAST(original_capacity, available_capacity + v_visitor_count),

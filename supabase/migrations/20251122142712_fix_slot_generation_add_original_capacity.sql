/*
  # Fix Slot Generation - Add original_capacity field

  1. Changes
    - Updates the INSERT statement to include the original_capacity field
    - This field tracks the initial capacity before any overbooking adjustments
*/
    ...
    IF EXTRACT(DOW FROM v_current_date)::integer = ANY(v_days_of_week) THEN
      ...
      LOOP
        ...
        WHILE v_slot_start_minutes + v_duration_minutes <= v_shift_end_minutes LOOP
          ...
          -- Insert slot with original_capacity
          INSERT INTO slots (
            tenant_id,
            shift_id,
            employee_id,

-- Fix get_visitor_export_summary: total_spent must only include PAID bookings.
-- Unpaid bookings must NOT be included in total_spent.
  ...
  SELECT
    ...
    COALESCE(SUM(b.total_price) FILTER (
      WHERE COALESCE(b.package_covered_quantity, 0) = 0
        ...
        AND LOWER(b.payment_status::text) IN ('paid', 'paid_manual')
...
COMMENT ON FUNCTION get_visitor_export_summary(uuid, uuid, text) IS
  'Returns aggregated booking counts and total spent for a visitor. total_spent excludes package-covered and UNPAID bookings (only paid/paid_manual).';

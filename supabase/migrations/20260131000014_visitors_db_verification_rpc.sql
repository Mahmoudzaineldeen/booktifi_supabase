-- Visitors page: database-level verification RPC for debugging.
-- Returns Q1–Q6 stats as JSON. Use GET /api/debug/visitors-db-check to compare with API.
...
BEGIN
  ...
  -- Q6: Total spent (paid only, status confirmed/completed/checked_in)
  SELECT COALESCE(SUM(total_price), 0) INTO v_total_spent
  FROM bookings
  WHERE tenant_id = p_tenant_id
    AND COALESCE(package_covered_quantity, 0) = 0

...
-- ============================================================================
-- This migration creates the create_bulk_booking function
-- which allows creating multiple bookings in a single atomic transaction
-- with strict availability validation to prevent overbooking
-- ============================================================================
...
BEGIN
  ...
  -- CRITICAL: This entire block is atomic - if any booking fails, all rollback
  -- ============================================================================
  FOR v_slot_index IN 1..array_length(p_slot_ids, 1) LOOP
    -- Get slot again (already locked from validation step)
    -- Re-verify availability just before booking (double-check after lock)
    SELECT id, available_capacity, is_available, original_capacity, booked_count, tenant_id
    INTO v_slot_record
    FROM slots
    WHERE id = p_slot_ids[v_slot_index]
    FOR UPDATE; -- Re-lock to ensure no changes since validation

    -- CRITICAL: Final availability check just before booking
    -- This prevents race conditions where capacity changed between validation and booking
    SELECT COALESCE(SUM(reserved_capacity), 0)
    INTO v_locked_capacity
    FROM booking_locks
    WHERE slot_id = p_slot_ids[v_slot_index]
    ...
    IF v_available_capacity < 1 THEN
      RAISE EXCEPTION 'Slot % capacity changed during booking. Only % available now.', 
        p_slot_ids[v_slot_index], v_available_capacity;
    END IF;
...
ALTER FUNCTION public.create_bulk_booking(
  ...
  numeric, text, uuid, text, uuid, uuid, text, uuid
) OWNER TO postgres;

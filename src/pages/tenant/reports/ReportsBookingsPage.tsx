...
export function ReportsBookingsPage() {
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-4">{t('reports.bookings.title', 'Bookings report')}</h2>
      <Card className="mb-6">
        <CardContent className="p-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('visitors.service', 'Service')}</label>
              <select
                className="w-full px-3 py-2 border border-gray-300 rounded-lg dark:bg-gray-900 dark:border-gray-600 dark:text-gray-100"
                value={serviceId}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('visitors.bookingStatus', 'Booking Status')}</label>
              <select
                className="w-full px-3 py-2 border border-gray-300 rounded-lg dark:bg-gray-900 dark:border-gray-600 dark:text-gray-100"
                value={status}
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-gray-800/70">
                    <td className="px-3 py-2">
                      <div>{r.slot_date || '—'}</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400">{r.start_time ? formatTimeTo12Hour(r.start_time) : ''}</div>
                    </td>

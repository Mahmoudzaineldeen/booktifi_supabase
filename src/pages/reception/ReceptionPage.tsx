...
import { Calendar, Plus, User, Phone, Mail, Clock, CheckCircle, XCircle, LogOut, CalendarDays, DollarSign, List, Grid, ChevronLeft, ChevronRight, X, Package, QrCode, Scan, Download, FileText, Search, Ban, Wrench, UserX, BarChart3, ChevronDown, Edit } from 'lucide-react';
...
interface Booking {
  ...
  slot_id?: string;
  /** Assigned staff for employee-based bookings */
...
export function ReceptionPage() {
  ...
  // - service-slot mode: automatic
  const effectiveAssignmentMode: 'automatic' | 'manual' =
    isEmployeeBasedMode
      ? (tenantAssignmentMode === 'both' ? assignmentMode : (tenantAssignmentMode === 'manual' ? 'manual' : 'automatic'))
      : 'automatic';
  ...
  // Mode defaults: for tenant mode "both" keep receptionist choice; fixed modes are enforced.
  useEffect(() => {
    if (isEmployeeBasedMode) {
      if (tenantAssignmentMode === 'manual') {
        setAssignmentMode('manual');
      } else if (tenantAssignmentMode === 'automatic') {
  ...
  async function searchBookings(type: SearchType, value: string) {
    ...
    try {
      ...
      const transformedBookings = (result.bookings || []).map((b: any) => ({
        ...
        required_slot_count: b.required_slot_count ?? null,
        ...
        paid_quantity: b.paid_quantity ?? 0,
        package_subscription_id: b.package_subscription_id ?? null,
        services: b.services || { name: '', name_ar: '' },
        slots: b.slots || { slot_date: '', start_time: '', end_time: '' },
        employee_id: b.employee_id ?? b.users?.id ?? null,
        users: b.users || null,
  ...
  async function handleEditBooking(skipClose = false) {
    ...
    try {
      const API_URL = getApiUrl();
      const token = localStorage.getItem('auth_token');
      const pricingMeta = getEditPricingMeta(editingBooking as Booking, editSelectedTagId || (editingBooking as any).tag_id || '', editConsumeFromPackage);
      ...
      const updateData: any = {
        ...
        service_id: (editingBooking as any).service_id,
        tag_id: editSelectedTagId || (editingBooking as any).tag_id || null,
  ...
  async function openEditBooking(booking: Booking | BookingDetailsModalBooking) {
    ...
    const bookingEmpId =
      (booking as any).employee_id != null && String((booking as any).employee_id).trim() !== ''
        ? String((booking as any).employee_id)
        : (booking as any).users?.id != null && String((booking as any).users.id).trim() !== ''
          ? String((booking as any).users.id)
          : '';
  ...
  async function handleEditServiceChangeReception(nextServiceId: string) {
    ...
    setEditingBooking({ ...(editingBooking as Booking), service_id: nextServiceId } as Booking);
    setSelectedNewSlotId('');
    setChangeTimeEmployeeId('');
    await fetchEditTagsForService(nextServiceId);
    if (!nextServiceId) {
    ...
    await fetchTimeSlotsForEdit(
      ...
      userProfile.tenant_id,
      editingTimeDate,
      (editingBooking as any)?.id,
      editingBooking as Booking,
    );
  }

  function handleEditTagChangeReception(nextTagId: string) {
    setEditSelectedTagId(nextTagId);
  }

  function handleEditConsumeToggleReception(nextValue: boolean) {
  ...
  async function handleSaveEditBookingReception() {
    if (!editingBooking) return;
    const serviceChanged = !!editingOriginalBooking && (editingBooking as any).service_id !== (editingOriginalBooking as any).service_id;
  ...
  async function fetchTimeSlotsForEdit(
    serviceId: string,
    tenantId: string,
    date?: Date,
    excludeBookingId?: string,
    /** Use this booking for default employee (state may not be flushed yet when opening edit). */
    bookingForEmployeeDefault?: Booking | BookingDetailsModalBooking | null,
  ) {
    ...
    try {
      ...
      if (needEmpPicker && empSource) {
        const bid =
          (empSource as any).employee_id != null && String((empSource as any).employee_id).trim() !== ''
            ? String((empSource as any).employee_id)
            : (empSource as any).users?.id != null && String((empSource as any).users.id).trim() !== ''
              ? String((empSource as any).users.id)
              : '';
  ...
  async function handleTimeDateChange(newDate: Date) {
    ...
    if (editingBooking && userProfile?.tenant_id) {
      await fetchTimeSlotsForEdit(
        (editingBooking as any).service_id,
        userProfile.tenant_id,
        newDate,
        editingBooking.id,
        editingBooking as Booking,
  ...
  useEffect(() => {
    if (!editingBooking) return;
    const chosenTagId = editSelectedTagId || (editingBooking as any).tag_id || '';
    const meta = getEditPricingMeta(editingBooking as Booking, chosenTagId, editConsumeFromPackage);
    ...
    setEditBookingForm((prev) => ({ ...prev, total_price: meta.totalPrice }));
  }, [editingBooking?.id, (editingBooking as any)?.service_id, editingBooking?.visitor_count, editSelectedTagId, editConsumeFromPackage, editPricingTags, editCustomerPackages, services]);
  ...
  async function fetchCustomerPackagesByPhoneForEdit(fullPhoneNumber: string): Promise<CustomerPackage[]> {
  ...
  async function fetchEditTagsForService(serviceId: string) {
    if (!serviceId || !userProfile?.tenant_id) {
      setEditPricingTags([]);
      setEditSelectedTagId('');
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`${getApiUrl()}/tags/by-service/${serviceId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json().catch(() => ({}));
      const tags = (data?.tags || []) as typeof editPricingTags;
      setEditPricingTags(tags);
      setEditSelectedTagId((prev) => {
        if (prev && tags.some((x) => x.id === prev)) return prev;
        return tags[0]?.id || '';
      });
    } catch {
      setEditPricingTags([]);
  ...
  function getEditTagFee(tagId: string): number {
    const selected = editPricingTags.find((x) => x.id === tagId);
    if (!selected || selected.is_default) return 0;
    return Math.max(0, Number(selected.fee_value ?? 0));
  }

  function getEditPricingMeta(targetBooking: Booking, tagId: string, consumePkg: boolean) {
    const service = services.find((s) => s.id === (targetBooking as any).service_id);
    ...
    const remaining = editCustomerPackages.reduce((sum, pkg) => {
      const usage = pkg.usage.find((u) => u.service_id === (targetBooking as any).service_id);
      return sum + Math.max(0, Number(usage?.remaining_quantity ?? 0));
    }, 0);
    const packageCoveredQty = consumePkg ? Math.min(qty, remaining) : 0;
    const paidQty = Math.max(0, qty - packageCoveredQty);
    const totalPrice = paidQty * unitPrice + tagFee;
    const packageSubscription = consumePkg
      ? editCustomerPackages.find((pkg) =>
          pkg.usage.some((u) => u.service_id === (targetBooking as any).service_id && Number(u.remaining_quantity || 0) > 0)
        )
      : null;
    return {
      unitPrice,
  ...
  function BookingCard({ booking, compact = false }: { booking: Booking; compact?: boolean }) {
    ...
    return (
      <div
        className={`rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden transition-shadow hover:shadow-md border-l-4 h-full ${statusBorder}`}
      >
        <div className={compact ? 'p-4 space-y-3' : 'p-5'}>
          {/* Top row: avatar, customer, time, badges */}
          <div className={`flex items-start gap-4 ${compact ? 'flex-col' : 'flex-wrap'}`}>
            <div className="flex items-center gap-3 min-w-0 flex-1 w-full">
              <div className={`flex shrink-0 items-center justify-center rounded-full bg-slate-100 text-slate-700 font-semibold ${compact ? 'h-9 w-9 text-base' : 'h-11 w-11 text-lg'}`}>
                {initial}
              </div>
          ...
          <div className={`grid gap-3 ${compact ? 'grid-cols-2' : 'mt-4 grid-cols-2 sm:grid-cols-4'}`}>
          ...
          {booking.notes && (
            <div className="mt-3 rounded-lg bg-amber-50/80 border border-amber-100 px-3 py-2">
              ...
              <p className={`text-sm text-amber-900 ${compact ? 'line-clamp-1' : 'line-clamp-2'}`}>{booking.notes}</p>
            </div>
          )}

          {/* Invoice row */}
          <div className={`mt-4 flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-gray-100 ${compact ? 'items-start' : ''}`}>
              ...
              <div className="flex flex-wrap items-center gap-3 text-sm min-w-0">
                {booking.zoho_invoice_id && (
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-gray-600 font-mono text-xs break-all">{booking.zoho_invoice_id}</span>
                    <Button
                ...
                {booking.daftra_invoice_id && (
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-gray-600 font-mono text-xs break-all">{booking.daftra_invoice_id}</span>
                    <Button
  ...
  const bookingsToDisplay = (showSearchResults ? searchResults : (activeTab === 'today' ? todayBookings : bookings)).filter(
        ...
        <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex gap-2 flex-wrap">
            <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
              ...
              >
                <span className="hidden sm:inline">{i18n.language?.startsWith('ar') ? 'عرض الشبكة' : (t('bookings.gridView') || 'Grid')}</span>
              </Button>
              <Button
        ...
        <form onSubmit={(e) => {
          ...
          {selectedService && (
            <>
              ...
              {/* Employee-based mode assignment (same behavior as admin create booking). */}
                    ...
                    {(tenantAssignmentMode === 'both' || tenantAssignmentMode === 'automatic') && (
                      <label className={`flex items-start gap-3 p-3 rounded-lg border-2 cursor-pointer transition-colors ${
                        ...
                        <input
                          type="radio"
                          name="assignmentMode"
                          checked={effectiveAssignmentMode === 'automatic'}
                          onChange={() => {
                            setAssignmentMode('automatic');
                            setSelectedSlot('');
                            setSelectedEmployee('');
                    ...
                    {(tenantAssignmentMode === 'both' || tenantAssignmentMode === 'manual') && (
                      <label className={`flex items-start gap-3 p-3 rounded-lg border-2 cursor-pointer transition-colors ${
                        ...
                        <input
                          type="radio"
                          name="assignmentMode"
                          checked={effectiveAssignmentMode === 'manual'}
                          onChange={() => {
                            setAssignmentMode('manual');
                            setSelectedSlot('');
                            setSelectedEmployee('');
              ...
              {editingBooking.status !== 'cancelled' && editingBooking.status !== 'completed' && (
                <div className="space-y-4 mb-6 pt-4 border-t">
                  ...
                  <div>
                    ...
                    <select
                      value={(editingBooking as any).service_id || ''}
                      onChange={(e) => void handleEditServiceChangeReception(e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md"
                    >
                      {services.map((s) => (
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => handleEditTagChangeReception(e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md"
                      ...
                      {editLoadingPricingTags ? (
                        ...
                        <>
                          {editSelectedTagId && !editPricingTags.some((x) => x.id === editSelectedTagId) && (
                            <option value={editSelectedTagId}>{t('tags.currentTag', 'Current tag')}</option>
                          )}
                  ...
                  <div className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
                    <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                      <input
                        ...
                        onChange={(e) => handleEditConsumeToggleReception(e.target.checked)}
                        className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                      />
                      <span>{t('reception.consumeFromPackage')}</span>
                    </label>
                    <p className="mt-1 text-xs text-gray-600">
                      {(() => {
                        const meta = getEditPricingMeta(editingBooking as Booking, editSelectedTagId || (editingBooking as any).tag_id || '', editConsumeFromPackage);
                    ...
                    <div>
                      ...
                      >
                        ...
                        {(() => {
                          ...
                          const bEmpRaw =
                            editingBooking &&
                            ((editingBooking as any).employee_id != null &&
                            String((editingBooking as any).employee_id).trim() !== ''
                              ? String((editingBooking as any).employee_id)
                              : (editingBooking as any).users?.id != null &&
                                  String((editingBooking as any).users.id).trim() !== ''
                                ? String((editingBooking as any).users.id)
                                : '');
                          const bEmp = bEmpRaw || '';
                          if (bEmp && !empMap.has(bEmp)) {
                            const u = (editingBooking as any).users;
                            empMap.set(bEmp, {
                              id: bEmp,
                              name: u?.full_name || t('bookings.currentStaff', 'Current staff'),
                              name_ar: u?.full_name_ar ?? '',
                            });
                          }

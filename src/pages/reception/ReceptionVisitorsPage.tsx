          ...
          {loading ? (
            ...
            <>
                ...
                <table className="w-full text-sm min-w-[900px]">
                  <thead>
                    <tr className="bg-slate-100/80 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider border-b border-slate-200">
                      <th className="px-4 py-3.5 w-12"><span className="sr-only">{t('visitors.selectForExport', 'Select for export')}</span></th>
                      ...
                      {canBlockUnblock && <th className="px-4 py-3.5">{t('common.actions', 'Actions')}</th>}
                    </tr>
                  </thead>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 align-middle">
                            {isGuest ? <span className="text-gray-400 text-xs">—</span> : (
                              <span className="inline-block font-mono text-xs text-slate-600 bg-slate-100 px-2 py-1 rounded max-w-[140px] truncate" title={row.id}>{row.id}</span>
                            )}
                          </td>
                          <td className="px-4 py-3 align-middle">
                            <div className="flex items-center gap-3">
                              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-200 text-slate-700 font-semibold text-sm">{initial}</div>
                              <div className="min-w-0">
                                <p className="font-medium text-gray-900 truncate">{row.customer_name || '—'}</p>
                                {isGuest && (
                                  <span className="inline-flex mt-0.5 text-[10px] font-medium text-amber-700 bg-amber-100 px-1.5 py-0.5 rounded">{t('visitors.guest', 'Guest')}</span>
                                )}
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="px-4 py-3 align-middle">
                            <span className="inline-flex items-center gap-1.5 text-gray-600 truncate max-w-[160px]" title={row.email || undefined}>
                          ...
                          <td className="px-4 py-3 align-middle text-center">
                            <span className="inline-flex items-center justify-center min-w-[2rem] px-2 py-0.5 rounded-md bg-amber-100 text-amber-800 text-sm font-medium tabular-nums">{row.package_bookings_count}</span>
                          </td>
                          <td className="px-4 py-3 align-middle text-center">
                            <span className="inline-flex items-center justify-center min-w-[2rem] px-2 py-0.5 rounded-md bg-emerald-100 text-emerald-800 text-sm font-medium tabular-nums">{row.paid_bookings_count}</span>
                          </td>
                          <td className="px-4 py-3 align-middle">
                          ...
                          <td className="px-4 py-3 align-middle">
                            <span className={`inline-flex px-2.5 py-1 rounded-lg text-xs font-semibold ${row.status === 'blocked' ? 'bg-red-100 text-red-800' : 'bg-emerald-100 text-emerald-800'}`}>
                          ...
                          {canBlockUnblock && (
                            <td className="px-4 py-3 align-middle" onClick={(e) => e.stopPropagation()}>
                              {row.type === 'guest' ? (
                                ...
                                <Button variant="primary" size="sm" icon={<CheckCircle className="w-3 h-3" />} onClick={() => handleUnblock(row)} disabled={blockingId === row.id}>
                                  {t('visitors.unblock', 'Unblock')}
                                </Button>
                              ) : (
                                <Button variant="secondary" size="sm" icon={<Ban className="w-3 h-3" />} onClick={() => handleBlock(row)} disabled={blockingId === row.id}>
              ...
              {pagination.totalPages > 1 && (
                <div className="flex justify-between items-center px-5 py-4 border-t border-gray-200 bg-gray-50/50">
              ...
              {canBlockUnblock && detailVisitor.visitor.type === 'customer' && (
                <div className="mt-4 pt-4 border-t border-gray-200 flex gap-2">
                  {detailVisitor.visitor.status === 'blocked' ? (
                    <Button variant="primary" size="sm" icon={<CheckCircle className="w-4 h-4" />} onClick={() => handleUnblock({ ...detailVisitor.visitor, id: detailVisitor.visitor.id } as VisitorRow)} disabled={blockingId === detailVisitor.visitor.id} className="rounded-lg">
                      {t('visitors.unblock', 'Unblock Visitor')}
                    </Button>
                  ) : (
                    <Button variant="secondary" size="sm" icon={<Ban className="w-4 h-4" />} onClick={() => handleBlock({ ...detailVisitor.visitor, id: detailVisitor.visitor.id } as VisitorRow)} disabled={blockingId === detailVisitor.visitor.id} className="rounded-lg border-red-200 text-red-700 hover:bg-red-50">

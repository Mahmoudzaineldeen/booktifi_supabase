...
class ZohoService {
  ...
  /**
   * Map booking data to Zoho invoice format (shared business rules in unifiedInvoiceMapper).
   */
  async mapBookingToInvoice(bookingId: string): Promise<ZohoInvoiceData> {
    console.log(`[ZohoService] 🔍 Mapping booking ${bookingId} to invoice data (unified mapper)...`);
    const { mapBookingToUnifiedInvoice, unifiedToZohoPayload } = await import('./invoices/unifiedInvoiceMapper.js');
    const unified = await mapBookingToUnifiedInvoice(bookingId);
    const invoiceData = unifiedToZohoPayload(unified) as ZohoInvoiceData;

    let c = invoiceData.currency_code || 'SAR';
    if (!c || c.trim().length === 0) c = 'SAR';
    else c = c.trim().toUpperCase();
    if (!/^[A-Z]{3}$/.test(c)) c = 'SAR';
    invoiceData.currency_code = c;
    ...
    console.log(
      `[ZohoService]    Total amount: ${invoiceData.line_items.reduce((sum, item) => sum + (item.rate * item.quantity), 0)} ${invoiceData.currency_code}`
    );

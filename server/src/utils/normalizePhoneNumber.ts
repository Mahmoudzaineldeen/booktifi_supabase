/**
 * Normalize phone number to E.164-like format for supported countries.
 *
 * Supported special handling:
 * - Egypt (+20): remove local trunk zero after country code.
 * - Saudi (+966): remove local trunk zero after country code.
 */
export function normalizePhoneNumber(value: string): string | null {
  ...
  const normalizeSaudiLocal = (digitsAfterCode: string): string | null => {
    // Local with trunk zero: 05xxxxxxxx (10 digits) -> +9665xxxxxxxx
    if (digitsAfterCode.startsWith('0') && digitsAfterCode.length >= 10) {
      const withoutZero = digitsAfterCode.substring(1);
    ...
    // Local without trunk zero: 5xxxxxxxx (9 digits) -> +9665xxxxxxxx
